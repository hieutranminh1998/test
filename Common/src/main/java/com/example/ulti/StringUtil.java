...
public class StringUtil {
    ...
    public static String genRandomStringKey() {
        String upperCaseLetters = RandomStringUtils.random(6, 65, 90, true, true);
        String numbers = RandomStringUtils.randomNumeric(2, 7);
        String combinedChars = upperCaseLetters.concat(upperCaseLetters).concat(numbers);
        List<Character> pwdChars = combinedChars.chars().mapToObj(c -> (char) c).collect(Collectors.toList());
        Collections.shuffle(pwdChars);
        String privateKey = pwdChars.stream().collect(StringBuilder::new, StringBuilder::append, StringBuilder::append)
                .toString();
        return privateKey;
    }
